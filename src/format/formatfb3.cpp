...
void FormatFB3::parseMetadata(const qbrunzip* ZipData, const QString& entryName) const
{
    ...
    if (const QDomElement nodeBookTitle = descriptionXml.firstChildElement("fb3-description").firstChildElement("title")
                                                        .firstChildElement("main"); !nodeBookTitle.isNull())
    {
        book->metadata->Title = nodeBookTitle.text();
    }

    QDomNode relationsItem = descriptionXml.firstChildElement("fb3-description").firstChildElement("fb3-relations").
                                            firstChild();
    while (!relationsItem.isNull())
    {
        if (QDomElement authorElement = relationsItem.toElement(); !authorElement.isNull() && authorElement.
            attribute("link", "").compare("author") == 0)
        {
            ...
            if (book->metadata->Author.isEmpty())
            {
                book->metadata->Author = nameTitle;
            }
            else
            {
                book->metadata->Author.append(", ");
                book->metadata->Author.append(nameTitle);
            }
        }
...
QDomNode FormatFB3::parseBodyNode(const QDomNode& currentNode, const QMap<QString, QString>& bodyBinaries,
                                  int* tocIndex, QList<QBRTocItem>* tocList)
{
    ...
    case QDomNode::ElementNode:
        {
            ...
            else if (returnTagName.compare("a") == 0)
            {
                ...
                {
                    if (const QString aType = currentNode.attributes().namedItem("type").nodeValue(); aType.
                        compare("note") == 0)
                    {
            ...
            if (currentNode.hasChildNodes())
            {
                for (int i = 0; i < currentNode.childNodes().count(); i++)
                {
                    ...
                    returnValue.appendChild(parseBodyNode(localNode, bodyBinaries, tocIndex, &tocItems));
                }
            }
            ...
            if (currentNodeTag == "section")
            {
                QString tocItemTitle = currentNode.firstChildElement("title").text().simplified();
                if (tocItemTitle.isEmpty())
                {
                    tocItemTitle = tr("...");
                }
                ...
                {
                    tocItemAnchor = returnValue.attribute("id");
                }
                else
                {
...
QList<QDomNode> FormatFB3::parseBody(const qbrunzip* zipData, const QString& bodyEntryName) const
{
    ...
    for (int i = 0; i < relsData.count(); i++)
    {
        ...
        bodyBinaries.insert(
            nodeId, QString("data:%1;base64,%2").arg(nodeContentType, zipData->getFileData(nodeTarget).toBase64()));
    ...
    {
        QDomNode convertedNode = parseBodyNode(docBodies.at(i), bodyBinaries, &tocIndex, &book->metadata->Toc);
...
bool FormatFB3::parseFile(const qbrunzip* ZipData)
{
    ...
    for (int i = 0; i < relsData.count(); i++)
    {
        ...
        {
            book->metadata->Cover.loadFromData(ZipData->getFileData(nodeTarget));
        }
    }
    ...
    /*
        for (int i = 0; i < book->metadata->Toc.count(); i++)
        {
            QBRTocItem tocItem = book->metadata->Toc.at(i);
            qDebug() << tocItem.Title << tocItem.Anchor << tocItem.Childs.count();
        }
        */
...
QBRBook* FormatFB3::getBook() { return book; }
