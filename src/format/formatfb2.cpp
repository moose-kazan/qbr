...
QDomNode FormatFB2::parseXmlNode(const QDomNode& currentNode, const QHash<QString, QString>& fb2Binaries, int* tocIndex,
    ...
    case QDomNode::ElementNode:
        {
            ...
            if (currentNode.hasChildNodes())
            {
                for (int i = 0; i < currentNode.childNodes().count(); i++)
                {
                    ...
                    returnValue.appendChild(parseXmlNode(localNode, fb2Binaries, tocIndex, &tocItems));
                }
            }
...
bool FormatFB2::parseXml(const QByteArray& fileData)
{
    ...
    {
        QDomNode convertedNode = parseXmlNode(fb2Body, fb2Binaries, &tocIndex, &book->metadata->Toc);
...
void FormatFB2::parseBookInfo(const QDomDocument* parserXml) const
{
    ...
    for (int i = 0; i < nodeAuthors.count(); i++)
    {
        ...
        {
            book->metadata->Author = authorName;
        }
        else
        {
            book->metadata->Author += ", " + authorName;
    ...
    if (const QDomElement coverImageNode = nodeTitleInfo.firstChildElement("coverpage").firstChildElement("image"); !
        coverImageNode.isNull() && coverImageNode.hasAttribute("href"))
    {
        ...
        while (!binaryItem.isNull())
        {
            if (binaryItem.hasAttribute("id"))
            {
                ...
                {
                    book->metadata->Cover.loadFromData(QByteArray::fromBase64(binaryItem.text().toUtf8()));
                    break;
                }
            }
...
QBRBook* FormatFB2::getBook() { return book; };
